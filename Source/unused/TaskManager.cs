...
// reports on thingdefs with "quality" tag
//
public static class UpdateResources
{
    ...
    {
        interestingThingCategories = new[]
        {
            "Foods",
            "FoodMeals",
            "Drugs",
            ...
            .Select(name => DefDatabase<ThingCategoryDef>.GetNamed(name, false))
            .ToHashSet();
    }
    ...
    public static void Task(Map map)
    {
        ...
        if (amounts.Any() && total != lastTotal)
        {
            ...
            var amountList = amounts
                .Select(pair => $"{pair.Value} {pair.Key.label.CapitalizeFirst()}")
                .Join();
...
public static class ReportColonistThoughts
{
    public static void Task(Map map)
    {
        if (AIItemsMod.Settings.reportColonistThoughts == false)
            return;
        ...
        foreach (var colonist in map.mapPawns.FreeColonists)
        {
            ...
            foreach (var thought in colonist.needs.mood.thoughts.memories.Memories)
            {
                ...
                if (thoughtCounts.ContainsKey(label))
                    thoughtCounts[label] = (
                        thoughtCounts[label].Count + 1,
                        thoughtCounts[label].MoodEffect + moodEffect
                    );
                else
            ...
            var formattedThoughts = thoughtCounts
                .Select(kvp =>
                {
                    var plural = kvp.Value.Count > 1 ? $"(x{kvp.Value.Count})" : "";
                    var moodEffectDescription = "";
                    if (kvp.Value.MoodEffect != 0.0f)
                        moodEffectDescription =
                            $"{(kvp.Value.MoodEffect > 0 ? " (improving mood by " : " (decreasing mood by ")}{Math.Abs(kvp.Value.MoodEffect)})";
                    return $"{kvp.Key}{plural}{moodEffectDescription}";
                })
                .Where(s => string.IsNullOrEmpty(s) == false);

            var thoughtsMessage =
                $"{colonist.Name.ToStringShort}'s recent thoughts: {formattedThoughts.Join()}";
...
public static class ReportColonistOpinions
{
    ...
    public static void Task(Map map)
    {
        if (AIItemsMod.Settings.reportColonistOpinions == false)
            return;
        ...
        foreach (var otherColonist in freeColonistsCopy)
        {
            ...
            {
                var trend =
                    currentOpinion == previousOpinion
                        ? "stayed the same"
                        : currentOpinion > previousOpinion
                            ? "improved"
                            : "worsened";
                opinionMessages.Add(
                    $"{colonist.Name} current opinion of {otherColonist.Name}: {currentOpinion} (has {trend} since yesterday)"
                );
            }
            else
                opinionMessages.Add(
                    $"{colonist.Name} current opinion of {otherColonist.Name}: {currentOpinion}"
                );
...
public static class UpdateEnergyStatus
{
    public static void Task(Map map)
    {
        ...
        if (!AIItemsMod.Settings.reportEnergyStatus)
            return;
        ...
        foreach (
            var powerPlant in map.listerBuildings.allBuildingsColonist.Where(building =>
                building.GetComp<CompPowerPlant>() != null
            )
        )
        {
            ...
            if (compPowerPlant.PowerOn)
            {
                ...
                //         PowerOutput = compPowerPlant.PowerOutput
                //     }
                // );
            }
        }
        ...
        foreach (
            var consumerBuilding in map
                .listerBuildings.allBuildingsColonist.Select(building =>
                    building.GetComp<CompPowerTrader>()
                )
                .Where(comp => comp != null && comp.PowerOutput < 0)
        )
        {
            if (consumerBuilding.PowerOn)
            {
                ...
                //     new EnergyConsumer
                //     {
                //         Label = consumerBuilding.parent.Label,
...
public static class UpdateRoomStatus
{
    public static void Task(Map map)
    {
        if (AIItemsMod.Settings.reportRoomStatus == false)
            return;
                        ...
                        foreach (var statPair in room.stats)
                            if (statPair.Key != null)
                                statsStringBuilder.Append(
                                    $"{statPair.Key.label} is {statPair.Value}, "
                                );

                    var statsString = statsStringBuilder.ToString().TrimEnd(',', ' ');

                    var article =
                        roleLabel.IndexOf("'s", StringComparison.OrdinalIgnoreCase) != -1
                            ? ""
                            : "A ";
                    var sentenceEnding = ", and ";
                    roomsList.Add(
                        $"{article}{roleLabel}, its "
...
public static class UpdateResearchStatus
{
    public static void Task(Map map)
    {
        if (AIItemsMod.Settings.reportResearchStatus == false)
            return;
        ...
        var completedResearchDefs = DefDatabase<ResearchProjectDef>
            .AllDefsListForReading.Where(r => r.IsFinished)
            .ToList();

        var availableResearchDefs = DefDatabase<ResearchProjectDef>
            .AllDefsListForReading.Where(r => !r.IsFinished && r.PrerequisitesCompleted)
            .ToList();
...
public static class UpdateColonistRoster
{
    public static void Task(Map map)
    {
        if (AIItemsMod.Settings.reportColonistRoster == false)
            return;
        ...
        foreach (var colonist in map.mapPawns.FreeColonists)
        {
            ...
            foreach (var hediff in colonist.health.hediffSet.hediffs)
            {
                ...
                )?.TryGetComp<HediffComp_Immunizable>();
                // colonistData.Health.Add(
                //     new HediffData
                //     {
                //         Label = hediff.Label,
